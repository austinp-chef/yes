...
public sealed class WeaponHolder : Component
{
	...
	[Property, Category( "Position" )] public Vector3 BarrelOffset { get; set; } = new Vector3( 65f, 12f, -0.9f );
	...
	private void CreateViewModel( WeaponPickup weapon )
	{
		...
		// Tint: hitscan weapons get black overlay, others use default
		if ( weapon.Type == WeaponPickup.WeaponType.Hitscan )
			renderer.Tint = new Color( 0.05f, 0.05f, 0.05f );
		else
			renderer.Tint = weapon.ModelTint;
	...
	private void HandleAttack()
	{
		...
		else if ( HeldWeapon.Type == WeaponPickup.WeaponType.Throwable )
		{
			if ( Time.Now - _lastFireTime >= 0.5f )
				ThrowGrenade();
		}
	}
	...
	private void Fire()
	{
		...
		renderer.RenderType = Sandbox.ModelRenderer.ShadowRenderType.Off;
		bolt.LocalScale = new Vector3( 0.2f, 0.03f, 0.03f );
	...
	private void FireHitscan()
	{
		...
		Sound.Play( "sounds/impacts/buildings/metal_gib-1.vsnd", CameraObject.WorldPosition );
		...
		if ( tr.Hit && tr.GameObject is not null )
		{
			...
			if ( health is not null )
				health.TakeDamage( HeldWeapon.HitscanDamage, camRot.Forward );
		}
	...
	private void SpawnTracer( Vector3 start, Vector3 end )
	{
		try
		{
			...
			tracerObj.LocalScale = new Vector3( dist, 0.01f, 0.01f );

			var renderer = tracerObj.Components.Create<ModelRenderer>();
			renderer.Model = Model.Load( "models/dev/box.vmdl" );
			renderer.Tint = new Color( 0.05f, 0.05f, 0.05f, 0.9f );
			renderer.RenderType = Sandbox.ModelRenderer.ShadowRenderType.Off;
	...
	private void ThrowGrenade()
	{
		...
		// Spawn grenade at barrel offset
		var spawnPos = camPos
			+ camRot.Forward * BarrelOffset.x
			+ camRot.Right * BarrelOffset.y
			+ camRot.Up * BarrelOffset.z;

		var nade = new GameObject( true, "Grenade" );
		nade.Tags.Add( "projectile" );
		nade.WorldPosition = spawnPos;
		nade.LocalScale = new Vector3( 0.15f, 0.15f, 0.15f );

		// Green cube
		var renderer = nade.Components.Create<ModelRenderer>();
		renderer.Model = Model.Load( "models/dev/box.vmdl" );
		renderer.Tint = new Color( 0.1f, 0.6f, 0.1f );
		renderer.RenderType = Sandbox.ModelRenderer.ShadowRenderType.On;

		// Grenade logic — throw forward and slightly up
		var throwVelocity = camRot.Forward * 800f + Vector3.Up * 250f;
		var projectile = nade.Components.Create<Grenade>();
		projectile.Launch( throwVelocity );

		// Remove from inventory after throwing
		Inventory[SelectedSlot] = null;
		DestroyViewModel();

		// Consume the world pickup too
		// (grenade is gone once thrown)

		Log.Info( "Threw grenade!" );
	}
	...
	private void DoMeleeHit()
	{
		...
		if ( tr.Hit && tr.GameObject is not null )
		{
			...
			if ( health is not null )
			{
				health.TakeDamage( HeldWeapon.MeleeDamage, camRot.Forward );
